...
export default class extends Controller {
  ...
  getCommandOutput(raw, command, parts) {
    ...
    return {
      html: `<span class="error-text">command not found: ${this.escapeHtml(command)}</span>\n<span class="dim">输入 <span class="syn-command">/help</span> 查看可用命令</span>`,
      error: true,
    };
  }
...
<span class="syn-flag">Packages:</span>  Rails, Hotwire, Turbo`;
  ...
  readArticle(cmd) {
    ...
    if (!slug)
      return {
        html: `<span class="dim">用法: <span class="syn-command">/read</span> <span class="syn-path">&lt;slug&gt;</span> · 输入 <span class="syn-command">/blog</span> 查看文章列表</span>`,
      };
    const blog = this.allBlogs.find((b) => b.slug === slug);
    if (!blog)
      return {
        html: `<span class="error-text">找不到文章: ${this.escapeHtml(slug)}</span>`,
        error: true,
      };
    ...
    if (blog.image)
      out += `<div class="terminal-img-frame" data-label="cover"><img src="${blog.image}" alt="${this.escapeHtml(blog.title)}" loading="lazy" /></div>`;
  ...
  openArticle(cmd) {
    ...
    if (!slug)
      return {
        html: `<span class="dim">用法: <span class="syn-command">/open</span> <span class="syn-path">&lt;slug&gt;</span></span>`,
      };
    const blog = this.allBlogs.find((b) => b.slug === slug);
    if (blog) {
    ...
    return {
      html: `<span class="error-text">找不到: ${this.escapeHtml(slug)}</span>`,
      error: true,
    };
  }
  ...
  restoreState() {
    ...
    if (chatMessages) {
      try {
        this.chatMessages = JSON.parse(chatMessages);
      } catch {
        this.chatMessages = [];
      }
    }
  ...
  renderMarkdown(text) {
    ...
    while (i < lines.length) {
      ...
      if (line.startsWith("```")) {
        ...
        if (closed) {
          result.push(
            `<pre style="background:#111;border:1px solid #1f1f1f;border-radius:6px;padding:10px 14px;margin:8px 0;overflow-x:auto;white-space:pre;"><code style="color:#a1a1aa;font-size:13px;">${codeLines.map((l) => this.escapeHtml(l)).join("\n")}</code></pre>`,
          );
        } else if (lang.startsWith("json:")) {
          result.push(
            '<div style="margin:4px 0;"><span style="color:#52525b;">加载中...</span></div>',
          );
        } else {
          result.push(
            `<pre style="background:#111;border:1px solid #1f1f1f;border-radius:6px;padding:10px 14px;margin:8px 0;overflow-x:auto;white-space:pre;"><code style="color:#a1a1aa;font-size:13px;">${codeLines.map((l) => this.escapeHtml(l)).join("\n")}</code></pre>`,
      ...
      if (line.match(/^[├└]── /)) {
        ...
        result.push(
          `<div style="margin:3px 0;"><span style="color:#3f3f46;">${branch}</span> ${this.inlineFormat(title)}</div>`,
        );
        i++;
        continue;
      }
      ...
      if (line.match(/^│\s+/)) {
        ...
        result.push(
          `<div style="margin:0 0 2px;"><span style="color:#3f3f46;">│</span>   <span style="color:#52525b;">${this.inlineFormat(content)}</span></div>`,
        );
        i++;
        continue;
      }
      ...
      if (line.startsWith("### ")) {
        result.push(
          `<div style="color:#60a5fa;font-weight:600;font-size:14px;margin:12px 0 4px;">${this.inlineFormat(line.slice(4))}</div>`,
        );
        i++;
        continue;
      }
      if (line.startsWith("## ")) {
        result.push(
          `<div style="color:#60a5fa;font-weight:600;font-size:15px;margin:14px 0 6px;">${this.inlineFormat(line.slice(3))}</div>`,
        );
        i++;
        continue;
      }
      if (line.startsWith("# ")) {
        result.push(
          `<div style="color:#60a5fa;font-weight:700;font-size:16px;margin:16px 0 8px;">${this.inlineFormat(line.slice(2))}</div>`,
        );
        i++;
        continue;
      }
      ...
      if (line.startsWith("> ")) {
        result.push(
          `<div style="border-left:3px solid #22c55e;padding:4px 12px;margin:6px 0;color:#a1a1aa;font-style:italic;">${this.inlineFormat(line.slice(2))}</div>`,
        );
        i++;
        continue;
      }
      ...
      if (line.match(/^[-*] /)) {
        result.push(
          `<div style="padding-left:14px;position:relative;margin:3px 0;"><span style="color:#22c55e;position:absolute;left:0;">▸</span>${this.inlineFormat(line.replace(/^[-*] /, ""))}</div>`,
        );
        i++;
        continue;
      }
      ...
      if (olMatch) {
        result.push(
          `<div style="padding-left:20px;position:relative;margin:3px 0;"><span style="color:#22c55e;position:absolute;left:0;font-weight:600;font-size:12px;">${olMatch[1]}.</span>${this.inlineFormat(olMatch[2])}</div>`,
        );
        i++;
        continue;
      }
  ...
  inlineFormat(text) {
    ...
    s = s.replace(
      /`([^`]+)`/g,
      '<code style="background:#1a1a1a;color:#fbbf24;padding:2px 6px;border-radius:3px;font-size:13px;">$1</code>',
    );
    s = s.replace(/\*\*(.+?)\*\*/g, '<span style="color:#e2e8f0;font-weight:600;">$1</span>');
    s = s.replace(/(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)/g, '<em style="color:#a1a1aa;">$1</em>');
    s = s.replace(
      /\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g,
      '<a href="$2" target="_blank" style="color:#5eead4;text-decoration:underline;text-underline-offset:2px;">$1</a>',
    );
    s = s.replace(
      /(?<!")(?<!=)(https?:\/\/[^\s<]+)/g,
      '<a href="$1" target="_blank" style="color:#5eead4;text-decoration:underline;text-underline-offset:2px;">$1</a>',
    );
    ...
    return parts
      .map((part, i) => {
        ...
        if (part.startsWith("--") || part.startsWith("-"))
          return `<span class="syn-flag">${this.escapeHtml(part)}</span>`;
        if (part.startsWith('"') && part.endsWith('"'))
          return `<span class="syn-string">${this.escapeHtml(part)}</span>`;
        if (part.includes("/") || part.includes("."))
          return `<span class="syn-path">${this.escapeHtml(part)}</span>`;
